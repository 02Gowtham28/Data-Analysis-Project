...
select category, item_purchased, count(item_purchased) as number_of_time_purchased,
row_number() over (partition by category order by count(item_purchased) desc) as purchased_rank
from customer
group by category, item_purchased
)
select category, item_purchased, number_of_time_purchased
from category_product
where purchased_rank <=3;
